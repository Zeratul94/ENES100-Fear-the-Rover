...
// Movement parameters
double fr_nav_input = .77 * 255;
double fl_nav_input = .7 * 255;
double br_nav_input = .77 * 255;
...
double rot_speed = 5000/3; // Get through testing, Units: no f*cking idea
...
void setup() {
  ...
  // Enes100.println(((Enes100.getTheta() - startheading));
...
void navigate_to_endzone() {
  ...
  if (!in_endzone) {
    if (true){//close_enough(heading, 0, 3)) { // If we're pointing the right way...
...
float usensor(int trigPin, int echoPin) {
  ...
  unsigned long duration = pulseIn(echoPin, HIGH, 20000); // Timeout for 200ms
